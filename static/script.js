...
const musicSheet = document.getElementById('osmdContainer');
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (typeof opensheetmusicdisplay !== "undefined") {
        console.log("OpenSheetMusicDisplay is available.");
        var osmd = new opensheetmusicdisplay.OpenSheetMusicDisplay("osmdContainer");

        fetch("/static/twinkle_twinkle.xml") // 서버에서 파일 불러오기 
            .then(response => {
                if (!response.ok) {
                    throw new Error("Network response was not ok " + response.statusText);
                }
                return response.text(); // 불러온 파일 txt로 변환 
            })
            .then(data => {
                osmd.load(data).then(() => osmd.render());
                console.log("music sheet update")
            })
            .catch(error => console.error('Error loading the MusicXML:', error));
    } else {
        console.error("OpenSheetMusicDisplay is not available.");
    }
});
                    ...
                    if (assignedNumber) {
                        if (!lastPlayed[trackId] || now - lastPlayed[trackId] > 1000) { // 1초 이상 지났는지 확인
                            if (playingTrackId !== trackId) {  // 현재 재생 중인 트랙 ID가 다른 경우
                                audio.src = 'thinkthing/new_sound_files/stretched_audio_${assignedNumber}.wav';
                                audio.play();
